...
SPECIAL_TARGETS = [TARGET_MYSQL, TARGET_POSTGRES, TARGET_SYSLOG, TARGET_SYS_STDOUT, TARGET_SYS_STDERR,
                   TARGET_SYS_STDOUT_ALIAS, TARGET_SYS_STDERR_ALIAS]
...
LOG_COLORS = {MSG_DEBUG: [WHITE, BLACK, FALSE], MSG_WARN: [WHITE, BLACK, FALSE], MSG_ERROR: [WHITE, BLACK, TRUE],
              MSG_INFO: [WHITE, BLACK, FALSE]}
...
from re import sub
from string import zfill, lower, upper, join, replace, split, strip
...
# default use
def log_init(log_name, log_file):
    loghd = Logger().get_instance(log_name)
    loghd.set_target(log_file)
    loghd.set_rotation(ROTATE_DAILY)
    loghd.set_loglevel(LOGLEVEL_NORMAL)
    return loghd
        ...
        if (not target in self.__Logger_targets):
            if (target == TARGET_MYSQL):
                if (mysql_available == TRUE):
                    ...
                    try:
                        self.__Logger_mysql_connection = MySQLdb.connect(host=args[0], db=args[1], user=args[2],
                                                                         passwd=args[3])
    ...
    def __Logger_showmessage(self, message, messagesource):
        ...
        for i in range(len(self.__Logger_targets)):
            ...
            if (target == TARGET_MYSQL):
                ...
                sqlStatement = "INSERT INTO %s (host, facility, level, date, time, program, msg) VALUES ('%s', '%s', '%s', %s, '%s', '%s')" % (
                    self.__Logger_mysql_tablename, self.hostname, self.__Logger_functionname, LOG_MSG[messagesource],
                    sqltime, str(self.__Logger_classname), sub("'", "`", message + " " + ndc))
            ...
            elif (target == sys.stdout) or (lower(target) == TARGET_SYS_STDOUT) or (
                        lower(target) == TARGET_SYS_STDOUT_ALIAS):
                sys.stdout.write("%s\n" % line)
            elif (target == sys.stderr) or (lower(target) == TARGET_SYS_STDERR) or (
                        lower(target) == TARGET_SYS_STDERR_ALIAS):
                sys.stderr.write("%s\n" % line)
            else:
                target.write("%s\n" % line)
